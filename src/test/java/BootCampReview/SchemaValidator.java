...
public class SchemaValidator extends TestBase{
    ...
    public void test2(){
        Response response = given().accept(ContentType.JSON)
                .when().get("/api/spartans/9")
                .then().statusCode(200)
                .and().body(JsonSchemaValidator.matchesJsonSchema(new File("C:\\Users\\Zulpikar\\IdeaProjects\\EU9_RestAssure_Review\\src\\test\\java\\BootCampReview\\singleSpartanSchema.json")))
                .extract().response();
    }
}
